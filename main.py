...
BUS_ADDR = "ipc://hotoe-bus.ipc"
APP_ID = "com.github.random-app" 
DEBUG_TOOLS = False
with open("manifest.json", 'r') as f: 
    d = json.load(f)
    APP_ID = d.get("id", "com.github.random-app")
    DEBUG_TOOLS = d.get("debug", False)
...
class HotoeEngine(Gtk.Application):
    ...
    def on_inspector_attach(self, inspector):
        self.system_input_regions = [[0, 0, self.monitor_w, self.monitor_h]]
...
def ensure_hhotkey_installed():
    ...
    if source_file.exists():
        ...
        print(f"[hotoe] hhotkey installed to {target_file}")
        return True
    return False
