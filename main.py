...
class HotoeEngine(Gtk.Application):
    ...
    def setup_hotkey_bus(self):
        context = zmq.Context()
        self.hotkey_pull = context.socket(zmq.PULL)
        self.hotkey_pull.bind("ipc://hotoe-hotkeys.ipc")
        threading.Thread(target=self.hotkey_listener, daemon=True).start()

    def hotkey_listener(self):
        while self.running:
            try:
                combo = self.hotkey_sub.recv_string(flags=zmq.NOBLOCK)
                match = self.hotkey_events.get(combo)  # combo -> event_name, populated after user confirms
                if match:
    ...
    def _on_hotkey_fired(self, widget, args, event_name):
        self.call_event_js(event_name, {})
        return True
    ...
    def _handle_register_hotkey(self, call_id, accelerator, event_name):
        hotkey_id = uuid.uuid4().hex[:8]

        

    def _handle_register_hotkey(self, call_id, accelerator, event_name):
        normalized = self._normalize_combo(accelerator)  # "Shift+S" -> "SHIFT+S"
        dialog = Gtk.AlertDialog()
        dialog.set_message("Global hotkey request")
        dialog.set_detail(f"This app wants to bind '{accelerator}' to once execute \n'hotoe-hotkey-daemon {normalized}'\n\nThis will be calling event named'{event_name}'\nIf you allow it, please add this bind to your config manually, we dont want to accidentally spoil it.")
        dialog.set_buttons(["Deny", "Allow"])
        dialog.set_default_button(0)

        def on_response(source, result, *_):
            try:
